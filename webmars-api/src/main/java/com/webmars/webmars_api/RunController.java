...
public class RunController {
    ...
    public RunResponse log(@Valid @RequestBody LogRunRequest req, @AuthenticationPrincipal String username) {
        ...
        // findByIdWithOwner: the visibility check below reads the owner,
        // which is LAZY — a plain findById proxy explodes outside the
        // session (the production 500 when logging runs of private
        // snippets).
        Snippet s = snippets.findByIdWithOwner(req.snippetId())
                .orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND));
        if (s.getVisibility() == Snippet.Visibility.PRIVATE
                && (s.getOwner() == null || !s.getOwner().getUsername().equals(username))) {
    ...
    public List<RunResponse> recent(@AuthenticationPrincipal String username,
        ...
        User me = users.findByUsername(username)
                .orElseThrow(() -> new ResponseStatusException(HttpStatus.UNAUTHORIZED));
        return runs.findRecentWithSnippetByUserId(me.getId(), PageRequest.of(0, Math.min(limit, 100)))
